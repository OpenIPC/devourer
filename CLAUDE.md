...
**Jaguar2.** A second, self-contained HAL under `src/jaguar2/` for **RTL8822BU**
(chip 8822B, 2T2R USB, `2357:012d` Archer T3U / `0bda:b82c` — the T3U reports
2T2R via `REG_SYS_CFG` bit 27). It is a hybrid of the other two: HalMAC firmware
download, MAC init and power sequencing follow the Jaguar3 path, while the
BB/AGC/RF register tables use the older phydm `check_positive` format (like
Jaguar1, via the shared `PhyTableLoader`). Bring-up is ported from the vendor
rtl88x2bu tree — power-on, DLFW, MAC/BB/RF init, LC calibration + IQK, RX, and
on-air TX across **2.4 and 5 GHz at 20/40/80 MHz** with per-rate, bandwidth-aware
efuse TX power. The `txpwr_lmt` regulatory-limit table is generated by
`tools/extract_8822b_txpwr_lmt.py` (worldwide-min, rfe_type-3 for the T3U); the
RF/AGC tune to the wide channel's **central** channel, and the RF18/LCK `CHNLBW`
value keeps the 5 GHz band bits (8/16) the vendor preserves. Single chip — no
strategy dispatch.
...
not Jaguar2. The Jaguar2 **8822BU/8812BU** USB parts are supported (see the
Jaguar2 HAL above); the **8822BE** (PCIe variant) and the **Kestrel (11ax)**
families remain out of scope — same "AU"/"BU" branding, different bus/baseband.
...
(requires JAGUAR1), `DEVOURER_JAGUAR2` (8822BU), `DEVOURER_JAGUAR3_8822C`,
...
The Jaguar2 HAL is a flat, single-chip set under `src/jaguar2/`
(`RtlJaguar2Device`, `HalJaguar2`, `HalmacJaguar2Fw`/`MacInit`,
`RadioManagementJaguar2`, `FrameParserJaguar2.h`, `Halrf8822b`) — same
`IRtlDevice` contract, no strategy interfaces. `HalJaguar2::set_channel_bw`
does channel + 20/40/80 MHz bandwidth (central-channel tune); `apply_tx_power`
programs the per-rate, bandwidth-aware efuse TXAGC clamped to the generated
`txpwr_lmt` table.
...
`hal/phydm/rtl8822{c,e}/` by `tools/extract_8822c_phy_tables.py`; the 8822B
tables under `hal/phydm/rtl8822b/` by `tools/extract_8822c_phy_tables.py`
(`--chip 8822b`) and the 8822B `Hal8822b_TxpwrLmt.h` regulatory limits by
`tools/extract_8822b_txpwr_lmt.py`. Edit the generators, not the output. The
runtime table walker is the shared `PhyTableLoader` (Jaguar1 + Jaguar2, each fed
by its own glue — `PhyTableLoaderJaguar2` supplies the 8822B arrays and apply
order) / `src/jaguar3/PhyTableLoaderJaguar3` (Jaguar3), not the upstream phydm
parser.
