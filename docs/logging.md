...
| `hopset.sense` | TX (`DEVOURER_TX_SENSE`) | t, v, role, slot, round, gen, base_idx, ch, phase (pre\|post), probe, window_us (MEASURED, barrier-read-end→read-end), settle_us, read_us, valid_fa + cca_ofdm/cca_cck/fa_ofdm/fa_cck + cca_rate/fa_rate (per ms), valid_igi + igi, valid_nhm + nhm_busy/nhm_env/nhm_dur, valid_clm + clm (emitted, not scored), flags "0x…" (`TxSenseFlag`), scored, occ — one quiet-window observation |
