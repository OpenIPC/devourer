...
| --- | --- | --- |
| 10 MHz | RF-inert: OBW99 16.43 MHz vs the 20 MHz control's 16.46, normal ~98% flood duty | partial: OBW99 11.89 MHz vs the 17.77 control — narrowed, but nowhere near half |
| 5 MHz | near-silent: flood airtime collapses to ~1% of capture blocks | **continuous 100%-duty emission** — no inter-frame gaps, a carrier-shaped radiator rather than packets; it stops only with session teardown (chip power-off) |
