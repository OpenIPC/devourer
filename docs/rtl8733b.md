...
- On-air TX throughput (`tests/bench_onair.py`, SDR duty × PHY rate, MCS7/20,
  ...
  5 GHz number.
- Occupied bandwidth (`tests/sdr_obw.py`, frame-gated average PSD): MCS7 at
  20 MHz reads OBW99 17.5 MHz on ch36 / 17.8 MHz on ch13, −20 dBr width
  21.0 MHz (ch36); at 40 MHz, OBW99 36.4 MHz on ch36 and 38.7 MHz on ch13
  (ambient-inflated), each at its correct HT40 centre (5190 / 2462 MHz) — the
  40 MHz centre-channel table checks out on both bands. The −20 dBr width at
  40 MHz is span-limited by the B210's 50 MHz capture; the full spectral
...
- The experimental 5/10 MHz sequence is measured non-functional on air: the
  10 MHz re-clock leaves the emission at the 20 MHz control's occupied
  bandwidth and the 5 MHz re-clock suppresses TX almost entirely (SDR
  evidence in "Narrowband status" below).
  `AdapterCaps::narrowband_ok` remains false.
...
(`tests/sdr_obw.py`, b733 unit, ch36, 6 Mbps flood) shows the path does not
produce narrowband RF: at 10 MHz the emission is as wide as the 20 MHz
control (OBW99 16.43 vs 16.46 MHz) with normal ~98% flood duty — the re-clock
is RF-inert — and at 5 MHz the flood's airtime collapses to ~1% of capture
blocks, the no-RF-output failure the vendor workaround itself describes. The
vendor's published capability mask remains 20/40 MHz. The code is retained
for further work, but as ported it is measured non-functional and the public
capability mask stays at 20/40 MHz.
