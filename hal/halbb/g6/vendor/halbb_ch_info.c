...
void halbb_ch_info_watchdog(struct bb_info *bb)
{
	...
	if (ch_physts->watchdog_mode == 0)
		return;

	/*[ch_info control]*/
	ch_physts->show_ch_info_cnt = 0;
	BB_DBG(bb, DBG_CH_INFO, "show_ch_info_cnt reset");
}
...
void halbb_ch_info_init(struct bb_info *bb)
{
	...
	ch_rpt->bb_ch_info_physts_i.watchdog_mode = false;
	ch_rpt->bb_ch_info_physts_i.show_ch_info_max_cnt = 5;
	ch_rpt->bb_ch_info_physts_i.show_ch_info_cnt = 0;
		       ...
		       char *output, u32 *_out_len)
{
	...
	u32 bitmap_type = 0; /*bitmap for physts type*/
	...
	if (_os_strcmp(input[1], "-h") == 0) {
		...
		BB_DBG_CNSL(*_out_len, *_used, output + *_used, *_out_len - *_used,
			    "trig_watchdog {force_bitmap_type} {show_ch_info_max_cnt} {watchdog_mode_en}\n\n");
	...
	if (_os_strcmp(input[1], "self_test") == 0) {
	...
	} else if (_os_strcmp(input[1], "trig_watchdog") == 0) {
		HALBB_SCAN(input[2], DCMD_DECIMAL, &val[0]);
		HALBB_SCAN(input[3], DCMD_DECIMAL, &val[1]);
		HALBB_SCAN(input[4], DCMD_DECIMAL, &val[2]);
		ch_physts->bitmap_type_auto_en = false;
		ch_physts->force_bitmap_type = val[0];
		bitmap_type |= BIT(ch_physts->force_bitmap_type);
		ch_physts->show_ch_info_max_cnt = (u16)val[1];

		if (bitmap_type & BIT(LEGACY_OFDM_PKT))
			cfg->ch_i_type = BB_CH_LEGACY_CH;
		else
			cfg->ch_i_type = BB_CH_MIMO_CH;
		...
		ch_physts->watchdog_mode = (bool)val[2];
		ch_rpt->print_en = (bool)val[2];
		if (ch_physts->watchdog_mode) {
			halbb_ch_trig_select(bb, CH_RPT_TRIG_ALWAYS);
			halbb_ch_info_physts_en(bb, true, bitmap_type, bb->bb_phy_idx);
		} else {
			halbb_ch_trig_select(bb, CH_RPT_TRIG_ONCE);
			halbb_ch_info_physts_en(bb, false, bitmap_type, bb->bb_phy_idx);
		}
