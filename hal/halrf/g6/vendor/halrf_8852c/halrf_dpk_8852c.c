	...
	u8 kidx)
{
	...
	RF_DBG(rf, DBG_RF_DPK, "[DPK] S%d thermal/ txagc_RF (K%d) = 0x%x/ 0x%x\n", path,
	...
	u8 loss_only)
{
	...
	do {
		...
		case 1: /*GAIN_LOSS and idx*/
			...
			pas_idx = _dpk_pas_read_8852c(rf, path, true);

			RF_DBG(rf, DBG_RF_DPK, "[DPK] pas_idx = %d\n", pas_idx);
	...
	enum rf_path path)
{
	...
	if (dpk->bp[path][kidx].band == BAND_ON_5G && dpk->bp[path][kidx].ch < 100)
		init_xdbm = 18;
	else if (dpk->bp[path][kidx].band != BAND_ON_24G)
		init_xdbm = 20;
	...
	init_xdbm = _dpk_kip_set_txagc_8852c(rf, phy, path, init_xdbm, true);
