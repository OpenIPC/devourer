...
__iram_func__
static void _iqk_sram_8852c(struct rf_info *rf, u8 path, u8 gp)
{
#ifdef  HALRF_DZ_LOG
	...
	#ifdef  HALRF_DZ_LOG
		if(path == RF_PATH_A)
			rfk_dz->iqk_dz_s0_rxsram[gp][i] = tmp;
		else
			rfk_dz->iqk_dz_s1_rxsram[gp][i] = tmp;
	#endif
	}
	...
	#ifdef	HALRF_DZ_LOG
		if(path == RF_PATH_A)
			rfk_dz->iqk_dz_s0_rxsram[gp][i+ 0x280] = tmp;
		else
			rfk_dz->iqk_dz_s1_rxsram[gp][i+ 0x280] = tmp;
	#endif
	}
				 ...
				 u8 path, u8 ktype)
{
	...
	case ID_RXK:		
		if (iqk_info->iqk_sram_en == true) {
			...
			_iqk_sram_8852c(rf, path, 0x2);
			_iqk_sram_8852c(rf, path, 0x3);
			}
		if (iqk_info->iqk_xym_en == true)
			_iqk_read_xym_8852c(rf, path);
		if (iqk_info->iqk_cfir_en == true) {
...
__iram_func__
static bool _iqk_nbrxk_bygp_8852c(struct rf_info *rf, enum phl_phy_idx phy_idx,
			      u8 path, u8 gp)
{
#ifdef  HALRF_DZ_LOG
	struct halrf_rfk_dz_rpt *rfk_dz = &rf->rfk_dz_rpt;
...
#ifdef  HALRF_DZ_LOG
	rfk_dz->iqk_dz_rx_xym[path][gp] = iqk_info->nb_rxcfir[path];
	rfk_dz->iqk_dz_rx_rxbb[path][gp] = rf0;
#endif
	if(kfail)
		_iqk_sram_8852c(rf, path, gp);
...
__iram_func__
static bool _iqk_nbtxk_bygp_8852c(struct rf_info *rf, enum phl_phy_idx phy_idx,
			      u8 path, u8 gp)
{
#ifdef  HALRF_DZ_LOG
	struct halrf_rfk_dz_rpt *rfk_dz = &rf->rfk_dz_rpt;
				...
				u8 path)
{
	...
	if(lok_is_fail) {
#ifdef  HALRF_DZ_LOG
		rfk_dz->iqk_dz_lok[path][0] = halrf_rrf(rf, path, 0x08, MASKRF);
		rfk_dz->iqk_dz_lok[path][1] = halrf_rrf(rf, path, 0x09, MASKRF);
		rfk_dz->iqk_dz_lok[path][2] = halrf_rrf(rf, path, 0x0a, MASKRF);
#endif
	...
	if (iqk_info->is_nbiqk) {
		iqk_info->iqk_rx_fail[0][path] =
			_iqk_nbrxk_bygp_8852c(rf, phy_idx, path, 0x2);
	} else {
		iqk_info->iqk_rx_fail[0][path] =
...
void iqk_preset_8852c(struct rf_info *rf, u8 path)
{
	...
	if (iqk_info->iqk_band[path] == BAND_ON_6G) { 
		if(idx == 0x0)
			...
			halrf_wreg(rf, 0x8110 + (path << 8), MASKDWORD, 0x11111111);
	} else {
		if(idx == 0x0)
			halrf_wreg(rf, 0x810c + (path << 8), MASKDWORD, 0x33112211);
		else
			halrf_wreg(rf, 0x8110 + (path << 8), MASKDWORD, 0x33112211);
	}
