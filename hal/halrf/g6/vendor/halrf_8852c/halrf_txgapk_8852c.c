...
//=========================
	...
	if(!txgapk_info->is_txgapk_ok) {
		for (i = 0; i < 17; i++)
			rfk_dz->txgapk_dz_track_d[path][i] = txgapk_info->track_d[path][i];
	}
#endif
...
//#endif
	...
	if(!txgapk_info->is_txgapk_ok) {
		for (i = 0; i < 17; i++)
			rfk_dz->txgapk_dz_power_d[path][i] = txgapk_info->power_d[path][i];
	}
#endif
}
...
#ifdef  HALRF_DZ_LOG
		if(!txgapk_info->is_txgapk_ok) {
			rfk_dz->txgapk_dz_code = (rfk_dz->txgapk_dz_code | BIT(path+8) | BIT(TXGAPK_IQKBK));
