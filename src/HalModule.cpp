...
bool HalModule::rtl8812au_hal_init() {
  ...
  /* Historic 8821 trace-derived post-fwdl block (hardcoded T2U Plus MAC at
   * 0x0610, ~13 trace-derived register pokes, BB/AGC value overrides,
   * RRSR/queue/RA-table writes — captured from aircrack-ng/88XXau on one
   * specific cold-init session) was removed here. PR #42 replaced the
   * 8821 init with a proper init-flow dispatch in HalModule (around line
   * 100-200), which sets up the chip from any starting state. Master had
   * been applying BOTH the proper init AND the trace replay; the trace
   * replay was load-bearing only before #42 landed, and its hard-coded
   * MAC + double-writes to 0x0520/0x0670 (different values per write)
   * make it actively harmful in the long run. Removed pending matrix +
   * Android-hotplug re-validation. */
