...
private:
  ...
  /* Decode the packed extended-header EFUSE into a logical map. Backs
   * read_efuse_rfe_type + read_efuse_txpwr_base. Walks the whole programmed
   * area — sections are NOT ordered by logical base, so stopping once a
   * requested offset is passed would drop later sections that backfill lower
   * offsets (see the .cpp for the measured case). */
