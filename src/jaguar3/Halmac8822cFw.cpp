...
bool Halmac8822cFw::iddma_dlfw(uint32_t src, uint32_t dest, uint32_t len,
  ...
  while (r32(REG_DDMA_CH0CTRL) & BIT_DDMACH0_OWN) {
    if (--cnt == 0) {
      _logger->error("Jaguar3 DLFW: ch0 not ready");
      return false;
    }
  }
 ...
 *
 * NB: only reachable after M2 power-on/queue-init (rtw_hal_init throws earlier
 * until then). _rsvd_boundary and the HIQ/beacon endpoint come from the M2 queue
 * allocation; QSEL_BEACON and OFFSET match the rsvd-page download convention. */
bool Halmac8822cFw::send_fw_page(uint16_t pg_addr, const uint8_t *chunk,
  ...
  bool sent = _device.bulk_send_sync(frame.data(),
                                     static_cast<int>(frame.size()), 1000) >= 0;
  bool status = sent;
