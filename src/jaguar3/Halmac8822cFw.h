 ...
 *
 * STATUS (M3): the register/DDMA/checksum/boot-poll control flow is ported and
 * compiles. The per-chunk transport — send_fw_page() (halmac send_fwpkt_88xx ->
 * dl_rsvd_page_88xx, a bulk-OUT of the chunk with an 8822C TX descriptor into
 * the HIQ reserved page) — is the remaining hookup; it depends on the 8822C TX
 * descriptor (FrameParser8822c, M5) and currently throws. Validated on hardware
 * at M3-complete via usbmon diff vs the kernel rtl88x2eu driver. */
class Halmac8822cFw {
public:
   ...
   * FW ready (REG_MCUFW_CTRL == 0xC078). Throws jaguar3_todo for the not-yet-
   * wired transport step. */
...
private:
  ...
  /* Transport seam — halmac send_fwpkt_88xx/dl_rsvd_page_88xx. TODO(M3/M5):
   * build the 8822C rsvd-page TX descriptor and bulk-OUT the chunk. */
   ...
   * allocation during power-on. TODO(M2): set from the txff allocation. */
