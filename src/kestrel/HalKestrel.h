...
private:
  ...
  /* ---- halrf_iqk_8852c.c: I/Q imbalance calibration (LOK + TXK + RXK). --------
   * Host-driven NCTL: each step writes BB/RF, fires a one-shot at BB 0x8000,
   * polls 0xbff8/0x80fc, reads the fail bit. All result-readback (xym/cfir/sram)
   * is disabled (matches iqk_init_8852c: iqk_*_en=false), so this is the direct
   * write+poll path. Runs per channel-set for RF_AB (non-DBCC). io_ofld=false =>
   * halrf_write_fwofld_start/end are no-ops and elided. iqk_get_ch_info's
   * band/bw/ch come from the current tune, passed in (no cur_chandef here). */
  void iqk_8852c(uint8_t band, uint8_t bw, uint8_t center_ch);

  /* halrf_config_8852c_nctl_reg: load the IQK/DPK/GapK calibration-engine
   * microcode (the generated array_mp_8852c_nctl_reg table) into the 0x8000
   * NCTL block after the RF tables and before any cal. Without it the one-shot
   * cal triggers fire at an unprogrammed engine. Handshake: 0x8000=8, poll
   * 0x8080==4; then walk {addr,val} pairs (0xfe..0xf9 = delay opcodes). */
  void config_nctl_reg_8852c();

private:
  /* Per-tune IQK state (subset of halrf_iqk_info; single-tune, non-MCC). */
  struct IqkState {
    uint8_t iqk_band[2] = {0, 0}; /* 0=2G,1=5G,2=6G */
    uint8_t iqk_bw[2] = {0, 0};   /* 0/1/2 = 20/40/80 */
    uint8_t iqk_ch[2] = {0, 0};
    uint8_t syn1to2 = 1; /* 1 non-DBCC, 3 DBCC */
    bool is_nbiqk = false;
    bool lok_cor_fail[2] = {false, false};
    bool lok_fin_fail[2] = {false, false};
    bool iqk_tx_fail[2] = {false, false};
    bool iqk_rx_fail[2] = {false, false};
    bool is_wb_txiqk[2] = {true, true};
    bool is_wb_rxiqk[2] = {true, true};
    uint32_t nb_txcfir[2] = {0x40000000, 0x40000000};
    uint32_t nb_rxcfir[2] = {0x40000000, 0x40000000};
    uint8_t iqk_table_idx[2] = {0, 0};
  } _iqk;

  /* IQK one-shot ktype opcodes (halrf_iqk_8852c.h ID_* enum, subset used). */
  enum IqkId {
    ID_TXAGC = 0,
    ID_A_FLoK_coarse,
    ID_G_FLoK_coarse,
  ...
  void iqk_doiqk_8852c(uint8_t path);        /* halrf_doiqk (per-path) */
  void iqk_get_ch_info_8852c(uint8_t path);  /* fills 0x9fe0/0x9fe4/0x9fe8 rpt */
  void iqk_macbb_setting_8852c(uint8_t path);
  void iqk_preset_8852c(uint8_t path);
  void iqk_by_path_8852c(uint8_t path);      /* LOK -> TXK -> RXK */
  void iqk_restore_8852c(uint8_t path);
  void iqk_afebb_restore_8852c(uint8_t path);
  void iqk_disable_rxagc_8852c(uint8_t path, uint8_t en);
  bool iqk_one_shot_8852c(uint8_t path, uint8_t ktype);
  bool iqk_check_cal_8852c(uint8_t path, uint8_t ktype);
  /* LOK */
  void lok_res_table_8852c(uint8_t path, uint8_t ibias);
  void iqk_txk_setting_8852c(uint8_t path);
  bool iqk_lok_8852c(uint8_t path);
  bool iqk_lok_2g_8852c(uint8_t path);
  bool iqk_lok_5g_8852c(uint8_t path);
  bool iqk_lok_6g_8852c(uint8_t path);
  bool lok_2g_finetune_check_8852c(uint8_t path);
  bool lok_5g_finetune_check_8852c(uint8_t path);
  /* TXK / RXK */
  bool txk_group_sel_8852c(uint8_t path);
  bool iqk_nbtxk_bygp_8852c(uint8_t path, uint8_t gp);
  void iqk_rxk_setting_8852c(uint8_t path);
  bool rxk_group_sel_8852c(uint8_t path);
  bool iqk_nbrxk_bygp_8852c(uint8_t path, uint8_t gp);
  bool iqk_check_nbiqc_8852c(uint8_t ktype, uint8_t path);
  /* halbb_ctrl_rx_cca_8852c + halbb_bb_reset_en_8852c (RX-CCA gate during IQK) */
  void bb_ctrl_rx_cca_8852c(bool cca_en, uint8_t band);
  void bb_reset_en_8852c(bool en, uint8_t band);

public:
  /* halrf_dpk_8852c: digital pre-distortion (TX linearity), per channel-set,
   * runs after TSSI in the RFK order. Thin wrapper driving the NCTL engine
   * (LOK/TXK/RXK-style one-shots). Additive + self-disabling: on any cal fail
   * it force-disables MDPD (halrf_dpk_onoff), so it never degrades TX below the
   * un-DPK'd path. Bypassed on a CAV-cut die at 5 GHz (the C8852C is CBV, so it
   * proceeds; guarded on read_cut()). max_dpk_txagc forced to 24 (the vendor's
   * own pwr_lmt_en==false short-circuit) so no TSSI/efuse power-limit data is
   * needed. RF_AB, non-DBCC. HalKestrelDpk.cpp. */
  void dpk_8852c(uint8_t band, uint8_t bw, uint8_t center_ch);

private:
  /* Per-tune DPK state (subset of halrf_dpk_info; single-tune, reload off). */
  struct DpkBkup {
    uint8_t band = 0, ch = 0, bw = 0;
    uint8_t path_ok = 0;
    uint8_t txagc_dpk = 0, ther_dpk = 0, gs = 0;
  };
  struct DpkState {
    uint8_t cur_idx[2] = {0, 0};
    uint8_t cur_k_set = 0;
    DpkBkup bp[2][2];
    bool is_limited_txagc[2] = {false, false};
    uint8_t limited_txagc[2] = {0, 0};
    uint8_t ov_flag[2] = {0, 0};
    uint8_t dpk_order[2] = {0, 0};
    uint8_t max_dpk_txagc[2] = {24, 24};
    bool is_dpk_enable = true;
  } _dpk;
  ...
  /* halrf_do_one_shot_8852c: the newer NCTL poll (2000-count, pre-enable). */
  void dpk_do_one_shot_8852c(uint8_t path, uint32_t cmd);
  void dpk_one_shot_8852c(uint8_t path, uint8_t id);
  void dpk_cal_select_8852c(uint8_t kpath);
  bool dpk_main_8852c(uint8_t path);
  bool dpk_agc_8852c(uint8_t path, uint8_t kidx, uint8_t init_xdbm);
  void dpk_bb_afe_setting_8852c(uint8_t path);
  void dpk_bb_afe_restore_8852c(uint8_t path);
  void dpk_rxagc_onoff_8852c(uint8_t path, bool on);
  void dpk_rf_setting_8852c(uint8_t path, uint8_t kidx);
  void dpk_kip_pwr_clk_onoff_8852c(bool on);
  void dpk_kip_control_rfc_8852c(uint8_t path, bool by_kip);
  void dpk_kip_preset_8852c(uint8_t path, uint8_t kidx);
  void dpk_kip_restore_8852c(uint8_t path);
  void dpk_txpwr_bb_force_8852c(uint8_t path, bool force);
  void dpk_tpg_sel_8852c(uint8_t path, uint8_t kidx);
  uint8_t dpk_kip_set_txagc_8852c(uint8_t path, uint8_t dbm, bool from_bb);
  void dpk_kip_set_rxagc_8852c(uint8_t path);
  void dpk_kset_query_8852c(uint8_t path);
  void dpk_para_query_8852c(uint8_t path, uint8_t kidx);
  bool dpk_sync_check_8852c(uint8_t path, uint8_t kidx);
  uint16_t dpk_dgain_read_8852c();
  void dpk_lbk_rxiqk_8852c(uint8_t path);
  void dpk_bypass_rxiqc_8852c(uint8_t path);
  uint8_t dpk_gainloss_8852c(uint8_t path, uint8_t kidx);
  uint8_t dpk_gainloss_read_8852c();
  uint8_t dpk_pas_read_8852c(uint8_t path);
  void dpk_set_mdpd_para_8852c(uint8_t path, uint8_t order);
  void dpk_idl_mpa_8852c(uint8_t path, uint8_t kidx);
  void dpk_gain_normalize_8852c(uint8_t path, uint8_t kidx);
  void dpk_on_8852c(uint8_t path, uint8_t kidx);
  void dpk_onoff_8852c(uint8_t path, bool off);
