...
#   2. write-back: for a sample of words in each window, poke the word with
...
SAMPLE=${SAMPLE:-8}   # words sampled per path window for the write-back leg
...
python3 - "$wb" "$PID" <<'PY'
import re, sys
rows = [(int(m.group(1), 16), int(m.group(2), 16)) for m in
        (re.match(r'^0x([0-9a-fA-F]{4}):\s+([0-9a-fA-F]{8})\s*$', l) for l in open(sys.argv[1])) if m]
if not rows:
    print("FAIL: no read-back rows parsed"); sys.exit(1)
