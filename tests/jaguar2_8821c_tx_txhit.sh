#!/usr/bin/env bash
# M7 validation (functional): does the RTL8821C (CF-811AC) TRANSMIT on air?
...
if [ "$HITS" -ge 1 ]; then
    echo "M7 FUNCTIONAL PASS: 8821C transmitted frames over the air"
else
    echo "M7 TX not confirmed (no hit). TX log /tmp/8821c-txside.log, RX $RXLOG"
fi
